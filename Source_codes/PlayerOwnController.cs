...
public class PlayerOwnController : MonoBehaviour {
	public int posX;
	public int posY;
	public int defPosX;
	public int defPosY;
	...
	public bool canMove(int x,int y) {
		return gamecontroller.GetComponent<PlayOwnController>().plocha.transform.GetChild(y).transform.GetChild(x).GetComponent<PlayOwnGridTileController> ().isPassable();
	}


	public bool canDrop2(int x,int y) {
		return gamecontroller.GetComponent<PlayOwnController>().plocha.transform.GetChild(y).transform.GetChild(x).GetComponent<PlayOwnGridTileController> ().canDrop();
	}
	...
	public void setPosition(int x=1, int y=6) {
		...
		this.transform.SetParent(gamecontroller.GetComponent<PlayOwnController> ().plocha.transform.GetChild (y).transform.GetChild (x).transform) ;
	...
	public IEnumerator move(string d, List<int> indexOfActual, int i) {
		...
		while (canDrop2 (posX, posY) && canMove (posX, posY + 1)) {
			this.transform.SetParent (gamecontroller.GetComponent<PlayOwnController> ().plocha.transform.GetChild (posY).transform.GetChild (posX).transform);
			this.GetComponent<RectTransform> ().localPosition = new Vector3 (0f, 0f, 0f);
	...
	public IEnumerator go(string sequence, List<int> indexOfActual) {
		...
		// FINISH
		if(this.transform.parent.GetComponent<PlayOwnGridTileController>().tileType == "o") {
			//Debug.Log("finish");
